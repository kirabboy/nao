    ...
    <section class="customer-fill-info">
        ...
        <form id="editAddressShipping" action="{{ route('update.address.shipping', $address_shipping->id ) }}" class="form-customer-info" method="post">
            @csrf
            @method("PUT")
            <div class="form-group">
                <label><h3>Tên khách hàng</h3></label>
                <input type="text" class="form-control btn-rounded" name="fullname" value="{{ $address_shipping->fullname }}">
            </div>
            <div class="form-group">
                <label><h3>Số điện thoại</h3></label>
                <input type="text" class="form-control btn-rounded" name="phone" value="{{ $address_shipping->phone }}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select name="province_id" class="form-control btn-rounded" required>
                    ...
                    <option {{ selected($address_shipping->province_id, $value->matinhthanh) }} value="{{ $value->matinhthanh }}">{{ $value->tentinhthanh }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="district_id" class="form-control btn-rounded" required>
                    ...
                    <option {{ selected($address_shipping->district_id, $value->maquanhuyen) }} value="{{ $value->maquanhuyen }}">{{ $value->tenquanhuyen }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="ward_id" class="form-control btn-rounded" required>
                    ...
                    <option {{ selected($address_shipping->ward_id, $value->maphuongxa) }} value="{{ $value->maphuongxa }}">{{ $value->tenphuongxa }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="warehouse_id" class="form-control btn-rounded" required>
                    @foreach($warehouse as $value)
                        <option {{ selected($address_shipping->warehouse_id, $value->id) }} value="{{ $value->id }}">{{ $value->name }}</option>
                    @endforeach
                </select>
            </div>
